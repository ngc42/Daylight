 ...
 * A UC is read with slotAddUserCalendarFromStorage() from storage.
 * If you add a calendar, a unique ID (type int, so not endless many ids!) is given.
 * There is a short curcuit created in MainWindow:
 *   signalUserCalendarDataModified()  --> Storage::slotUserCalendarDataModified()
...
public slots:
    // called from storage at startup
    void slotAddUserCalendarFromStorage(UserCalendarInfo* & info);
