...
CalendarScene::CalendarScene(const SettingsData & settings, QObject *parent) :
    QGraphicsScene(parent), m_currentBaseDate(QDate::currentDate()), m_weekStartDay(1), m_showView(CalendarShow::SHOW_UNKNOWN)
{
...
void CalendarScene::setAppointmentsForYear(const QList<Appointment*> & list)
{
    for(DayInYearItem* d : m_daysInYearItems)
        d->clearAppointments();
    if(list.isEmpty())
        return;
    QList<Appointment*> currentList;
    QList<Appointment*> nextList;
    ...
    while( ! nextList.isEmpty())
    {
        ...
        endDate = nextList[0]->endDateTime().date();
        nextList.removeAt(0);

        for(Appointment* apm : nextList)
        {
            if(apm->startDateTime().date() > endDate)
            {
                currentList.append(apm);
                endDate = apm->endDateTime().date();
                nextList.removeOne(apm);
            }
        }
...
void CalendarScene::setAppointmentsForMonth(const QList<Appointment*> & list)
{
    for(DayInMonthItem* d : m_daysInMonthItems)
        d->clearAppointments();
    if(list.isEmpty())
...
void CalendarScene::setAppointmentsFor3Weeks(const QList<Appointment*> & list)
{
    for(DayInMonthItem* d : m_daysIn3WeeksItems)
        d->clearAppointments();
    if(list.isEmpty())
...
void CalendarScene::setAppointmentsForWeek(const QList<Appointment*> & list)
{
    for(DayInWeekItem* d : m_daysInWeekItems)
        d->clearAppointments();
    if(list.isEmpty())
...
void CalendarScene::setAppointmentsForDay(const QList<Appointment*> & list)
{
    m_dayInDayItem->clearAppointments();
    if(list.isEmpty()) return;

    QList<Appointment*> fullDayList;
    QList<Appointment*> partDayList;

    // dispatch all appointments to fullDay and part-time-list.
    for(Appointment* apm : list)
    {
        if(apm->startDateTime().date() < m_currentBaseDate and apm->endDateTime().date() > m_currentBaseDate)
            fullDayList.append(apm);
        else if(apm->m_appointmentData.m_allDay)
            fullDayList.append(apm);
        else
            partDayList.append(apm);
    }
    // send them to item
...
void CalendarScene::createDays()
{
    ...
    for(int i = 0; i < 12; i++)
    {
        ...
        for(int k = 0; k < daysInMonth[i]; k++)
        {
            ...
            connect(tmp, SIGNAL(signalReconfigureAppointment(int)), this, SIGNAL(signalReconfigureAppointment(int)));
            connect(tmp, SIGNAL(signalDeleteAppointment(int)), this, SIGNAL(signalDeleteAppointment(int)));
    ...
    connect(m_dayInDayItem, SIGNAL(signalReconfigureAppointment(int)), this, SIGNAL(signalReconfigureAppointment(int)));
    connect(m_dayInDayItem, SIGNAL(signalDeleteAppointment(int)), this, SIGNAL(signalDeleteAppointment(int)));
